...
#import 25 us stocks
tickers = ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'META', 'NVDA', 'PYPL', 'NFLX', 'ADBE', 
           'INTC', 'CSCO', 'CMCSA', 'PEP', 'AVGO', 'COST', 'AMGN', 'TXN', 'QCOM', 'AMD', 
           'TMUS', 'HON', 'INTU', 'ISRG', 'SBUX']
...
class DataRepository:
  ...
  def fetch_tickers(self, min_date=None):
    ...
    for i,ticker in enumerate(tq):
      ...
      historyPrices['growth_future_10d'] = historyPrices['Adj Close'].shift(-10) / historyPrices['Adj Close']
      historyPrices['growth_future_20d'] = historyPrices['Adj Close'].shift(-20) / historyPrices['Adj Close']
      ...
      historyPrices['is_positive_growth_10d_future'] = np.where(historyPrices['growth_future_10d'] > 1, 1, 0)
      historyPrices['is_positive_growth_20d_future'] = np.where(historyPrices['growth_future_20d'] > 1, 1, 0)

      # Exponential Moving Average 10 days and 20 days
      historyPrices['EMA10'] = historyPrices['Close'].ewm(span=10, adjust=False).mean()
      historyPrices['EMA20'] = historyPrices['Close'].ewm(span=20, adjust=False).mean()

      # Volume features
      historyPrices['Volume_SMA10'] = historyPrices['Volume'].rolling(10).mean()
      historyPrices['Volume_SMA20'] = historyPrices['Volume'].rolling(20).mean()
  ...
  def fetch_indexes(self, min_date=None):
    ...
    # FTSE - FTSE Real Time Price. Currency in USD
    ftse_daily = yf.download(tickers = "^FTSE",
                     start = min_date,          
                    #  period = "max",
                     interval = "1d")
    # sleep 1 sec between downloads - not to overload the API server
    ...
    # https://finance.yahoo.com/quote/SMH/history?p=SMH
    # VanEck Semiconductor ETF (SMH) : NasdaqGM - Nasdaq Real Time Price USD
    smh_etf_daily = yf.download(tickers = "SMH",
                     start = min_date,            
                    #  period = "max",
                     interval = "1d")
    # sleep 1 sec between downloads - not to overload the API server
    ...
    ftse_daily_to_merge = self._get_growth_df(ftse_daily, 'ftse')
    dji_daily_to_merge = self._get_growth_df(dji_daily, 'dji')
    smh_etf_daily_to_merge = self._get_growth_df(smh_etf_daily, 'smh')
